...
class SequenceAssembler():
    def __init__(self, data_path, label, seq_len):
    ...
    def get_sequence(self):
        ...
        try:
            ...
            print(seq_start, seq_end, self.step)
            print(data.shape)
            print(sequence.shape)
            quit()
        self.seq_counter += 1
        if seq_end == self.elements_amount + self.step - 1:
            self.seq_counter = 0
...
def train():
    ...
    device = mx.gpu()
    net = StairSafetyNetLSTM(LSTM_layer_size, LSTM_num_layeres, LSTM_dropout, SEQUENCE_LEN)
    ...
    for epoch in range(EPOCHS):
        ...
        sequence_assemblers = [SequenceAssembler(data_path, train_data_paths[data_path], SEQUENCE_LEN) 
